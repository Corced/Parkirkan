...
export default function PetugasDashboard() {
    ...
    return (
        <div className="space-y-12 pb-20">
            ...
            {/* Quick Stats */}
            <StaggerContainer className="grid gap-8 md:grid-cols-2" staggerDelay={0.1} delay={0.1}>
                {statCards.map((card, idx) => (
                    <StaggerItem key={idx}>
                        <Card className={cn("border-t-8 shadow-sm hover:shadow-xl transition-all rounded-[2.5rem]", card.borderColor)}>
            ...
            {isEndShiftDialogOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-[2rem] p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-100">
                        <div className="space-y-4 text-center pb-2 border-b border-slate-100 mb-4">
                            <div className="h-16 w-16 bg-slate-900 rounded-full mx-auto flex items-center justify-center mb-2">
                                <LogOut className="h-8 w-8 text-white" />
                            </div>
                            <h3 className="text-2xl font-black text-black">Konfirmasi Akhiri Shift</h3>
                            <p className="text-slate-600 font-medium">Anda akan mengakhiri shift <span className="font-bold text-black bg-slate-100 px-2 py-1 rounded">{currentShift}</span>.</p>
                        </div>

                        <div className="space-y-3 pt-2 text-left mb-6">
                            <label className="text-sm font-black text-slate-700">Catatan Tambahan (Opsional)</label>
                            <textarea
                                className="w-full h-28 p-4 border-2 border-slate-200 rounded-2xl bg-slate-50 text-sm focus:border-blue-500 focus:bg-white transition-colors resize-none"
                                placeholder="Contoh: Izin pulang lebih awal karena sakit..."
                                value={endShiftNotes}
                                onChange={(e) => setEndShiftNotes(e.target.value)}
                            />
                            <p className="text-[11px] font-bold text-slate-400">Gunakan jika Anda mengakhiri shift secara darurat/sebelum waktunya.</p>
                        </div>
                        ...
                        <div className="flex gap-3 pt-2">
                            <Button
                                ...
                                className="flex-1 rounded-2xl h-14 font-black border-2 hover:bg-slate-50 text-slate-700"
                            >
                                Batal
                            </Button>
                            <Button
                                onClick={confirmEndShift}
                                className="flex-1 rounded-2xl h-14 bg-slate-900 hover:bg-black text-white font-black"
                            >
                                Ya, Akhiri
                            </Button>
                        </div>
